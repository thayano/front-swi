...
var element = document.getElementById('painelUsers');
var incluiDiv = '';

async function reqUser() {
    let reqUser = await fetch("http://179.124.9.84:7200/users/", {
        method: "GET",
        headers: {
    ...
    for (user of resUser){
        let reqProfile = await fetch("http://179.124.9.84:7200/profile/" + user.profile + "/", {
            method: "GET",
            headers: {
                Authorization:"token " + infoUser.token,
                Accept: "application/json",
        ...
        let reqCompanies = await fetch("http://179.124.9.84:7200/companies/" + user.companies + "/", {
            method: "GET",
            headers: {
                Authorization:"token " + infoUser.token,
                Accept: "application/json",
        ...
        <div class="app-card app-card-doc shadow-sm h-100 rounded-3" style="background-color: white;">
                 ...
                 <div class="app-card-actions">
                    <div class="dropdown">
                        ...
                        <ul class="dropdown-menu">
                            ...
                            <li>
                                <a data-bs-toggle="modal" data-bs-target="#editarUser" class="dropdown-item" onclick="editarUser(this.id)" href="#" id="${user.id}">
                            ...
                            <li>
                                <a class="dropdown-item" href="#" id="${user.id}" onclick="excluirUser(this.id)">
            ...
            <div class="app-card-body p-3 has-card-actions">
                <h4 class="app-doc-title truncate mb-0"><a href="#file-link">${user.name}</a></h4>
                <div class="app-doc-meta mt-3">
                    <ul class="list-unstyled mb-0" style="font-size:12px ;">
                        <li><span class="text-muted">Cargo:</span> ${resProfile.profile}</li>
                         <li><span class="text-muted">Empresa:</span> ${resCompanies.name}</li>
                        <li><span class="text-muted">ID:</span> ${user.id} </li>
                    </ul>
                </div><!--//app-doc-meta-->
                <!--//app-card-actions-->	
            </div><!--//app-card-body-->
...
async function excluirUser(id) {
    Swal.fire({
        ...
        text: "Vocẽ tem certeza que deseja excluir este Usuário?!",
        icon: 'warning',
...
async function confirExcluir(id) {
    let deleteUser = await fetch("http://179.124.9.84:7200/users/"+id+"/", {
        method: "DELETE",
        headers: {
            Authorization:"token " + infoUser.token,
            Accept: "application/json",
    ...
    let resUserDelete = await deleteUser.json();  
    if(deleteUser.status < 300){
        Swal.fire(
            'Pronto!',
            'Usuário foi excluido com sucesso.',
            'success'
          )
          document.location.reload(true);
    }
    else{
        Swal.fire("Erro!", "Ocorreu um erro ao exluir usuario. " +resUserDelete.message, "error");
    }
}
